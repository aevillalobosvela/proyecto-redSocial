<template>
    <div class="card border p-3" style="background-color: rgba(255, 255, 255, 0.726)">
        <div class="card-body" style="max-height: 475px; overflow-y: auto">
            <h1>Tendencia</h1>
            <div v-for="(publicacion, indexp) in topusersconpub" :key="indexp">
                <div class="card border mb-4">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-2">
                                <img v-if="publicacion.imagenurl" class="m-2 img-fluid" style="
                      border-radius: 15%;
                      border: solid 1px;
                      border-color: rgb(206, 158, 186);
                    " :src="publicacion.imagenurl" alt="User Image" width="100%" height="100%" />
                                <img v-else class="m-2 img-fluid" style="
                      border-radius: 15%;
                      border: solid 1px;
                      border-color: rgb(206, 158, 186);
                    " src="../assets/avatar.png" alt="User Image" width="100%" height="100%" />
                            </div>
                            <div class="col-4 mt-3">
                                <h5 style="margin: 0px">{{ publicacion.username }}</h5>
                                <p style="margin: 0px">
                                    <small>Hace {{ tiempoTranscurrido(publicacion.fec_pub) }}</small>
                                </p>
                            </div>
                            <div style="display: flex; justify-content: end" class="mt-3 col-6 text-end">
                                <p style="
                      font-size: 25px;
                      margin-right: 10px;
                      margin-top: 8px;
                    ...
                    ">
                                ...
                                <i v-if="comprobar_like(publicacion.cod_pub)" class="mt-2 thum fa fa-thumbs-up fa-2x"
                                    style="color: rgb(230, 155, 240)" v-on:click="quitarlike(publicacion.cod_pub)"></i>

                                <i v-else class="mt-2 thum fa fa-thumbs-up fa-2x"
                                    v-on:click="darlike(publicacion.cod_pub)"></i>
                            </div>
                        </div>
                    </div>
                    <div class="card-body" style="background-color: rgb(248, 238, 250)">
                        ...
                        <div class="text-center mb-2">
                            <img v-if="publicacion.imagenurlpub" class="text-center" :src="publicacion.imagenurlpub"
                                style="border-radius: 5%" alt="User Image" width="70%" height="50%" />
                        </div>
                    </div>
                    <ul class="list-group list-group-flush" style="max-height: 120px; overflow-y: auto">
                        <li class="list-group-item" v-for="(comentario, index) in comentarios[publicacion.cod_pub] ||
                            []" :key="index">
                            <div class="row mb-2 p-0">
                                <div class="col-1 p-0">
                                    <img v-if="rutaImagen(comentario)" class="p-0" style="
                        margin: 0px;
                        margin-left: 4px;
                        border-radius: 15%;
                        border: solid 1px;
                        border-color: rgb(206, 158, 186);
                      " :src="rutaImagen(comentario)" alt="User Image" width="100%" height="45px" />
                                    <img v-else class="img-fluid" style="
                        border-radius: 15%;
                        margin-left: 4px;
                        border: solid 1px;
                        border-color: rgb(206, 158, 186);
                        padding: 2px;
                      " src="../assets/avatar.png" width="100%" height="100%" />
                                </div>
                                <div class="col-11">
                            ...
                            <div class="row">
                                <div class="col-10">
                                    <input type="text" class="form-control" placeholder="Escribe un comentario..."
                                        :id="'comentario-' + publicacion.cod_pub" autocomplete="off" />
                                </div>
...
export default {
    computed: {
        ...mapState(["usuario", "publicaciones", "usersconpub", "likes_in_pub", "topusersconpub"]),
    },
    data() {
        return {
            com_publi: null,
    ...
    methods: {
        ...
        ...mapActions(["toprecuperarUsersconPub"]),
    ...
    async mounted() {
        ...
        await this.toprecuperarUsersconPub();
        this.fetchAllComments();
        this.recuperarUsersconPub();
        this.recuperarlikesinPubs();
        this.recuperarmislikes();
